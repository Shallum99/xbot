...
// Note: api.Client and api.RequestOptions are imported from xurl.
// The actual API call construction (SearchTriggerTweets, FetchTweet)
// lives in bot/api.go to avoid depending on xurl shortcut functions.
...
func FetchParentTweet(client api.Client, parentID string, opts api.RequestOptions) (*ParsedTweet, error) {
	resp, err := FetchTweet(client, parentID, opts)
	if err != nil {
